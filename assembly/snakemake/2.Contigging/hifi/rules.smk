rule hifiasm:
	...
	output:
		asm_p_fa = os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hifiasm/purgeL' + config['purgel'] + '/hifiasmL' + config['purgel'] + '.asm.p_ctg.gfa'),
		asm_a_fa = os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hifiasm/purgeL' + config['purgel'] + '/hifiasmL' + config['purgel'] + '.asm.a_ctg.gfa'),
	log:
...
rule hicanu:
	...
	input:
		hifi_fastq = expand("{sample}", sample=samples['file_path'])
	output:
		outdir = directory(os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hicanu')),
		outasm = os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hicanu/asm.contigs.fasta')
	log:
...
rule flye:
	...
	input:
		hifi_fastq = expand("{sample}", sample=samples['file_path'])
	output:
		outdir = directory(os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/flye')),
...
rule gfa_to_fasta:
	...
	input:
		asm_p_gfa = os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hifiasm/purgeL' + config['purgel'] + '/hifiasmL' + config['purgel'] + '.asm.p_ctg.gfa'),
		asm_a_gfa = os.path.join(config['snakemake_dir_path'], 'results/1.Contigging/hifiasm/purgeL' + config['purgel'] + '/hifiasmL' + config['purgel'] + '.asm.a_ctg.gfa')
	output:
