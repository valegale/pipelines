...
# Define the config.yaml citofonfile path
...
rule blast:
    ...
    shell:
        """
        (blastn -query {input.asm_p} -db {params.nt_db} -outfmt '6 qseqid staxids bitscore evalue std sscinames sskingdoms stitle' -max_target_seqs 10 -max_hsps 1 -evalue 1e-25 -num_threads {threads} -out {output.out_blast}) 2>> {log}
        """
...
rule blobtools:
    ...
    input:
        out_blast = os.path.join(config['snakemake_dir_path'],'asm.vs.nt.max10.1e25.blastn.out'),
        asm_p = os.path.join(config['results'], f'3.Purging/{prefix}.purged.fa'),
        map2cov = os.path.join(config['snakemake_dir_path'],'map2cov_blob.merged.bam.cov')# keep working!
    output:
        blob_json = os.path.join(config['results'], f'5.Decontamination/{prefix}_blob_output.blobDB.json'),
        blob_table = os.path.join(config['results'], f'5.Decontamination/{prefix}_table.blob_output.blobDB.table.txt'),

        blob_png = os.path.join(config['results'], f'5.Decontamination/{prefix}_plot.blob_output.blobDB.json.bestsumorder.phylum.p8.count.100.blobplot.cov0.png') 
#    log:
...
rule filter_asm:
    ...
    input:
        ...
        blob_table = os.path.join(config['results'], f'5.Decontamination/{prefix}_table.blob_output.blobDB.table.txt')
    output:
        contam_scaff = os.path.join(config['results'], f'5.Decontamination/{prefix}_contam_scaff.txt'),
        decont_asm = os.path.join(config['results'], f'5.Decontamination/{prefix}_asm_decontaminated.fasta')
#    log:
